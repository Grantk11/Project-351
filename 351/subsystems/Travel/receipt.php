...
<div id="wrapper">
    ...
    <div class="container">
        <h1>Submit Reimbursement for Trip</h1>
        <?php if ($error): ?>
            <div class="error"><?= htmlspecialchars($error) ?></div>
        <?php endif; ?>
        <form method="POST" action="">

            <label for="TripID">Trip ID:</label><br>
            <input type="number" id="TripID" name="TripID" min="1" required
                   value="<?= htmlspecialchars($_POST['TripID'] ?? $tripID ?? '') ?>"><br><br>

            <label for="Amount">Amount:</label><br>
            <input type="number" step="0.01" min="0.01" id="Amount" name="Amount" required
                   value="<?= htmlspecialchars($_POST['Amount'] ?? '') ?>"><br><br>
            <label for="BankInfo">Bank Info (6 digits):</label><br>
            <input type="text" id="BankInfo" name="BankInfo" maxlength="6" required
                   value="<?= htmlspecialchars($_POST['BankInfo'] ?? '') ?>"><br><br>
            <label for="Date">Date:</label><br>
            <input type="date" id="Date" name="Date" required
                   max="<?= date('Y-m-d') ?>"
                   value="<?= htmlspecialchars($_POST['Date'] ?? '') ?>"><br><br>
